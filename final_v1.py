...
# implicit
# implicit = pd.read_csv('data/predict_implicit_0.01_0.99.csv').values
# implicit[:,:2]-=1
# book_rating_train = np.concatenate([book_rating_train, implicit], axis=0)
...
for itera in range(0, 1):
	...
	c = keras.backend.constant(0.2)
	ratings = keras.layers.Concatenate(axis=-1)([ratings, user_bias, book_bias, user_info, book_info])# , record_bias
	ratings = Lambda(lambda x: x+c)(ratings)

	# bias = Add()([user_bias, book_bias])	
	# bias = Dense(units=35, activation='sigmoid')(bias)
	# ratings = BatchNormalization()(ratings)
	# bias = Dropout(0.15)(bias)
	# bias = Dense(units=1, activation='relu')(bias)
	# info = keras.layers.Concatenate(axis=-1)([user_info, book_info])
	# ratings = Lambda(lambda x: Add()([x, keras.backend.constant(0.3, shape=(1,1))]))(ratings)
	# 
	# c = K.repeat_elements(c,rep=1126,axis=0)
	# ratings = Add()([ratings, c])

	
	ratings = Dense(units=105, activation='sigmoid')(ratings)
	# ratings = BatchNormalization()(ratings)
	ratings = Dropout(0.1)(ratings)
	# ratings = Dense(units=512, activation='relu')(ratings)
	# # ratings = BatchNormalization()(ratings)
	# ratings = Dropout(0.55)(ratings)
	# output = Lambda(lambda x: x+2.5)(ratings)
	output = Dense(units=1, activation='relu')(ratings)
		
	# bias = Dense(units=25, activation='sigmoid')(bias)
	# # ratings = BatchNormalization()(ratings)
	# bias = Dropout(0.1)(bias)
	# bias = Dense(units=1, activation='relu')(bias)
	# output = Add()([ratings, bias])
	# info = Dense(units=115, activation='sigmoid')(info)
	# # ratings = BatchNormalization()(ratings)
	# info = Dropout(0.15)(info)
	# info = Dense(units=1, activation='relu')(info)
	# output = Add()([ratings, info])


	model = Model([user, book, user_info, book_info], output)
	# adam = Adam(lr=0.001, decay=0.0008/14) # decay=0.0008/14
	adam = Adam()
	model.compile(optimizer=adam, loss='mape', metrics=[])
	...
	matrix = model.fit([usersID, ISBN, users_info_train, books_info_train], train_rating, batch_size=1126, epochs=7,
	...
	if track == 1:
		...
		idx_upper = np.where((y_hat-y_floor)>=upper_threshold)[0] 
		idx_lower = np.where((y_hat-y_floor)<=lower_threshold)[0] 
		y_upper = y_hat[idx_upper].reshape(-1, 1)
		y_lower = y_hat[idx_lower].reshape(-1, 1)
		y_hat = np.concatenate([y_upper, y_lower], axis=0)
		book_rating_test_upper = book_rating_test[idx_upper]
		book_rating_test_lower = book_rating_test[idx_lower]
		book_rating_test = np.concatenate([book_rating_test_upper, book_rating_test_lower], axis=0)+1
		# isRound = np.where((y_hat-y_floor)>=round_threshold)[0]
		# notRound = np.where((y_hat-y_floor)<round_threshold)[0]
		# y_hat[isRound] = y_floor[isRound]+1
		# y_hat[notRound] = y_floor[notRound]
		...
		# print('std:', np.std(y_hat))
		# save
		predict_filename = 'predict_implicit.csv'
		y_hat = np.concatenate([book_rating_test, y_hat], axis=1)
		np.savetxt(predict_filename,y_hat,delimiter=",", fmt='%d,%d,%d')
